...
#                                   example: GRMOE
Variable.Condition = "A9FA" 
Control.Condition = "S9FA"
...
####Uses sigDEGs from primary comparison
cat.data = Enrichment(FDR[,2:1])

#cat.data = Subset.Enrichment(row.names(FDR), #row.names(FDR)[FDR[,2]<=0.05])
...
KEGG20(cat.data, FC[2:1], FDR[2:1], usecats)
...
plotkegg.genes(specific.kegg, FC[,2:1], FDR[,2:1], mean.data[,c(3,4,1,2)])
...
parent.category = 'lipid metabolic process'
