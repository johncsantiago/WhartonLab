...
#SBATCH -J GR-F1_STAR_UnsortedBAM.sh
...
#Submit cmd: sbatch -n 26 -t 16:00:00 --mem=64G GR-F1_STAR_UnsortedBAM.sh
...
STAR --genomeDir $stardir"second_index" --readFilesIn $sample1".fastq" $sample2".fastq" --runThreadN 20 --outSAMtype BAM Unsorted --outFileNamePrefix $stardir"Output_Files/BAM/"$sample

module load samtools/1.3.1
samtools index $stardir"Output_Files/BAM/"$sample"Aligned.out.bam"

module load python/3.7.4
module load htseq/0.11.1
htseq-count $stardir"Output_Files/BAM/"$sample"Aligned.out.bam" $genomedir"Drosophila_melanogaster.BDGP6.32.109.gtf" -t exon -s no -r pos -i gene_id -f bam > "/gpfs/data/kwharton/jsantiago/30-840029517/Count_Tables/"$sample"_CountTable_UnsortedBAM.txt"
