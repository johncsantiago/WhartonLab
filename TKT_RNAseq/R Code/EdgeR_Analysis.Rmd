...
cd.dir = "/Users/johncsantiago/Documents/GitHub/Datasets/GBB_Count_Tables/"

GBB.countdata = read.csv(paste0(cd.dir, list.files(cd.dir)[1]), 
                   sep = "\t", row.names = 1)
colnames(GBB.countdata) = substr(list.files(cd.dir)[1], 1, 7)

i=2
while(i <= length(list.files(cd.dir))){
  temp.cd = read.csv(paste0(cd.dir, list.files(cd.dir)[i]), 
                     sep = "\t", row.names = 1)
  colnames(temp.cd) = substr(list.files(cd.dir)[i], 1, 7)
  temp.cd[,1] = temp.cd[row.names(GBB.countdata),]
  GBB.countdata = cbind(GBB.countdata, temp.cd)
  i = i + 1
}

GBB.groups = read.csv("/Users/johncsantiago/Documents/GitHub/Datasets/GBB_Metadata.csv", row.names = 1)
