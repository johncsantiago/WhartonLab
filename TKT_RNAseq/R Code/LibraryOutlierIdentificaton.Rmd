...
GeneIDKey = read.csv("https://raw.githubusercontent.com/johncsantiago/WhartonLab/refs/heads/master/GeneralDataFiles/GeneIDKey.csv", row.names = 1)

#raw count data directory
cd.dir = "https://raw.githubusercontent.com/johncsantiago/WhartonLab/refs/heads/master/GeneralDataFiles/G85R_TKT_RNAseq_CountTables/"

countdata.files = read.csv(paste0(cd.dir, "CountTableFileNames.csv"), header = F)[,1]

countdata = read.csv(paste0(cd.dir, countdata.files[1]), 
                   sep = "\t", row.names = 1)
colnames(countdata) = substr(countdata.files[1], 1, 
                             (nchar(countdata.files[1]) - 15))

i=2
while(i <= length(countdata.files)){
  temp.cd = read.csv(paste0(cd.dir, countdata.files[i]), 
                   sep = "\t", row.names = 1)
  colnames(temp.cd) = substr(countdata.files[i], 1,
                            (nchar(countdata.files[i]) - 15))
  temp.cd[,1] = temp.cd[row.names(countdata),]
  countdata = cbind(countdata, temp.cd)
  i = i + 1
}



groups = read.csv(paste0(cd.dir,"All.TKT.metadata.csv"), row.names = 1)

#write.csv(countdata, "/Users/johncsantiago/Documents/GitHub/WhartonLab/20260701_G85R_TKT_RNAseq_FullAnalysis/RawCountData and Processing/AllCountData.csv")

#write.csv(groups, "/Users/johncsantiago/Documents/GitHub/WhartonLab/20260701_G85R_TKT_RNAseq_FullAnalysis/RawCountData and Processing/AllMetaData.csv")


##Clean Trait Data and calculate CPMs
countdata=GBB.countdata
groups=GBB.groups

groups = groups[groups$Experiment == "GBB-KO",]
...
MDSdata = plotMDS(z, 
                  top = 15000, 
                  col = as.numeric(factor(gr)), 
                  labels =  paste(groups$Group, groups$Replicate))
...
groups = groups[groups$Experiment == "GBB-OE",]
...
DEout=G_X_E$table
GRMDfxWTM3=DEout
sGRMDfxWTM3=DEout[DEout$FDR<.05,]
...
DEout=G_X_E$table
GRMDfxWTMDf3=DEout
sGRMDfxWTMDf3=DEout[DEout$FDR<.05,]
...
```{r echo = F, include = F}
###Generating new CPM, FDR and FC tables with males and females processed seperately

combined.cpmdata = matrix(0, ncol = ncol(cpmdata2), nrow = nrow(cpmdata2))
colnames(combined.cpmdata) = colnames(cpmdata2)
row.names(combined.cpmdata) = row.names(cpmdata2)

combined.cpmdata[row.names(cpmdata3), colnames(cpmdata3)] = cpmdata3
combined.cpmdata[row.names(cpmdata4), colnames(cpmdata4)] = cpmdata4

combined.FDR = matrix(0, ncol = 14, nrow = nrow(combined.cpmdata))
row.names(combined.FDR) = row.names(combined.cpmdata)

colnames(combined.FDR) = c('GRF.CxDf', 'GRF.CxOE', 
                           'WTF.CxDf', 'WTF.CxOE', 
...
combined.FDR[row.names(GR.F.CxDf3), 1] = GR.F.CxDf3[,'FDR']
combined.FDR[row.names(GR.F.CxOE3), 2] = GR.F.CxOE3[,'FDR']

combined.FDR[row.names(WT.F.CxDf3), 3] = WT.F.CxDf3[,'FDR']
combined.FDR[row.names(WT.F.CxOE3), 4] = WT.F.CxOE3[,'FDR']

combined.FDR[row.names(GRxWT.F3), 5] = GRxWT.F3[,'FDR']
combined.FDR[row.names(GRxWT.FDf3), 6] = GRxWT.FDf3[,'FDR']
combined.FDR[row.names(GRxWT.FOE3), 7] = GRxWT.FOE3[,'FDR']

combined.FDR[row.names(GR.M.CxDf3), 8] = GR.M.CxDf3[,'FDR']
combined.FDR[row.names(GR.M.CxOE3), 9] = GR.M.CxOE3[,'FDR']

combined.FDR[row.names(WT.M.CxDf3), 10] = WT.M.CxDf3[,'FDR']
combined.FDR[row.names(WT.M.CxOE3), 11] = WT.M.CxOE3[,'FDR']

combined.FDR[row.names(GRxWT.M3), 12] = GRxWT.M3[,'FDR']
combined.FDR[row.names(GRxWT.MDf3), 13] = GRxWT.MDf3[,'FDR']
combined.FDR[row.names(GRxWT.MOE3), 14] = GRxWT.MOE3[,'FDR']



combined.FC = matrix(0, ncol = 14, nrow = nrow(combined.cpmdata))
row.names(combined.FC) = row.names(combined.cpmdata)

colnames(combined.FC) = c('GRF.CxDf', 'GRF.CxOE', 
                          'WTF.CxDf', 'WTF.CxOE', 
                          'GRxWT.FC', 'GRxWT.FDf', 'GRxWT.FOE',
                          'GRM.CxDf', 'GRM.CxOE', 
                          'WTM.CxDf', 'WTM.CxOE', 
...
combined.FC[row.names(GR.F.CxDf3), 1] = GR.F.CxDf3[,'logFC']
combined.FC[row.names(GR.F.CxOE3), 2] = GR.F.CxOE3[,'logFC']

combined.FC[row.names(WT.F.CxDf3), 3] = WT.F.CxDf3[,'logFC']
combined.FC[row.names(WT.F.CxOE3), 4] = WT.F.CxOE3[,'logFC']

combined.FC[row.names(GRxWT.F3), 5] = GRxWT.F3[,'logFC']
combined.FC[row.names(GRxWT.FDf3), 6] = GRxWT.FDf3[,'logFC']
combined.FC[row.names(GRxWT.FOE3), 7] = GRxWT.FOE3[,'logFC']

combined.FC[row.names(GR.M.CxDf3), 8] = GR.M.CxDf3[,'logFC']
combined.FC[row.names(GR.M.CxOE3), 9] = GR.M.CxOE3[,'logFC']

combined.FC[row.names(WT.M.CxDf3), 10] = WT.M.CxDf3[,'logFC']
combined.FC[row.names(WT.M.CxOE3), 11] = WT.M.CxOE3[,'logFC']

combined.FC[row.names(GRxWT.M3), 12] = GRxWT.M3[,'logFC']
combined.FC[row.names(GRxWT.MDf3), 13] = GRxWT.MDf3[,'logFC']
combined.FC[row.names(GRxWT.MOE3), 14] = GRxWT.MOE3[,'logFC']


##write.csv(combined.cpmdata, "/Users/johncsantiago/Documents/GitHub/WhartonLab/TKT_RNAseq/CountTables/TKT_cpmdata.csv")
##write.csv(combined.FDR, "/Users/johncsantiago/Documents/GitHub/WhartonLab/TKT_RNAseq/CountTables/TKT.EdgeR.FDRTable.csv")
##write.csv(combined.FC, "/Users/johncsantiago/Documents/GitHub/WhartonLab/TKT_RNAseq/CountTables/TKT.EdgeR.FCTable.csv")
