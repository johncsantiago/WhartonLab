...
fullnetwork = function(metab, 
  ...
  while(i <= length(pathway)){
    if(pathway[1] != 'none'){
    temp.nodes = nodes[grep(pathway[i], nodes$Pathway),]
    temp.edges = unique(rbind(edges[edges$from %in% nodes$id,],
                         edges[edges$to %in% nodes$id,]))
    }
    if(i == 1){
      trimmed.nodes = temp.nodes
      trimmed.edges = temp.edges
    }
    if(i>1){
      trimmed.nodes = unique(rbind(trimmed.nodes, temp.nodes))
      trimmed.edges = unique(rbind(trimmed.edges, temp.edges))
    }
    if(i == length(pathway)){
      nodes = trimmed.nodes
      edges = trimmed.edges
  ...
  visNetwork(nodes, edges, background = "white", main = title)%>%
    ...
    #visLayout(hierarchical=F, improvedLayout=T)%>%
    visOptions(highlightNearest = TRUE, nodesIdSelection = list(enabled = T, values = nodes[nodes$size > 0, "label"]))%>%
    visNodes(font = list(color = "black", size = 20, background = "white"))%>%
...
```{r echo = F, warning=F, fig.height=5, fig.width=15}
...
##genes/metabolites with fold change below the cutoff are not colored red/blue
fccutoff = log2(1.25)
##Pathway options: "Purine", "PPP", "Glycolysis", "NAD", "TCA"
pathway = c("Purine")

fullnetwork(metab, enzyme, title, fccutoff, meansumcutoff, pathway)
...
pathway = c("Glycolysis", "TCA")
  ...
  pathway.genes = unique(edges$FBgn)
  pathway.genes = intersect(pathway.genes, row.names(G85R.cpm))
  hmdata = G85R.cpm[pathway.genes,]
  row.names(hmdata) = GeneIDKey[pathway.genes, "Symbol"]
  
  col.order = G85R.meta
  col.order = col.order[order(c(col.order$Sex), decreasing = F),]
  col.order = col.order[order(c(col.order$Genotype), decreasing = T),]
  col.order = col.order[order(c(col.order$TKT), decreasing = F),]

  col.order = col.order[order(c(col.order$Genotype), decreasing = T),]
    col.order = col.order[order(c(col.order$Sex), decreasing = F),]
    
    col.order=col.order[col.order$Sex=="F",]
    
    hmdata = hmdata[,col.order$SampleID]
    i=1
    while(i<=ncol(hmdata)){
      hmdata[hmdata[,i]<5,i] = NA
      i=i+1
    }
    
    i=1
    while(i<=nrow(hmdata)){
      hmdata[i,]=t(scale(t((hmdata[i,]))))
      i=i+1
    }
...
pathway = c("Glycolysis", "TCA")
  ...
  col.order = col.order[order(c(col.order$Genotype), decreasing = T),]
    ...
    col.order=col.order[col.order$Sex=="M",]
